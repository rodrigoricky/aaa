...
const SOURCE_COLUMNS = ['qty', 'new_qty', 'end_qty', 'balance'];
const TYPE_CANDIDATES = ['balance', 'new_qty', 'end_qty', 'qty'];
...
function isSupportedNumericType(column) {
    return [
        'bigint',
        'decimal',
        'float',
...
function chooseOldBalanceType(columns) {
...
function buildBackfillSql(table, columns) {
...
function printBackfillSql(table, columns) {
...
async function getDatabaseInfo(executor) {
...
async function findInventoryAdjustmentTable(executor) {
    const result = await executor
        .request()
        .input('tableName', sql_server_js_1.sql.NVarChar(128), TABLE_NAME)
        .query(`
      SELECT
...
async function getColumns(executor, table) {
    const result = await executor
        .request()
        .input('schemaName', sql_server_js_1.sql.NVarChar(128), table.schemaName)
        .input('tableName', sql_server_js_1.sql.NVarChar(128), table.tableName)
        .query(`
      SELECT
...
async function countRows(executor, table) {
    const result = await executor.request().query(`
...
async function countOldBalanceNullRows(executor, table, columns) {
    const oldBalance = columnSql(columns, OLD_BALANCE_COLUMN);
    const result = await executor.request().query(`
...
async function getSampleRows(executor, table, columns) {
...
async function inspectPreflight(pool) {
...
async function printDryRunPlan(pool, info) {
...
async function runMigration(pool, preflight) {
    const transaction = new sql_server_js_1.sql.Transaction(pool);
    let transactionActive = false;
    try {
        await step('Begin transaction', async () => {
            await transaction.begin(sql_server_js_1.sql.ISOLATION_LEVEL.READ_COMMITTED);
...
async function main() {
    ...
    console.log(`NODE_ENV: ${env_js_1.env.NODE_ENV}`);
    if (env_js_1.env.NODE_ENV === 'production' && !options.dryRun && !options.confirm) {
        console.error('Production database detected. Re-run with --confirm after taking a backup.');
        process.exitCode = 1;
        return;
    }
    const pool = await step('Connect to database', async () => (0, sql_server_js_1.getSqlPool)());
...
main()
    ...
    .finally(async () => {
    await (0, sql_server_js_1.closeSqlPool)().catch((error) => {
