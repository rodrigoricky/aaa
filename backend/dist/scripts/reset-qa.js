...
const reset_qa_service_js_1 = require("../modules/quantity-adjustments/reset-qa.service.js");
function printUsage() {
...
async function main() {
    const target = (0, reset_qa_service_js_1.resolveResetQaTargetFromCli)();
    if (!target) {
        printUsage();
        process.exitCode = 1;
        return;
    }
    const plan = await (0, reset_qa_service_js_1.inspectResetQuantityAdjustments)(target);
    ...
    const result = await (0, reset_qa_service_js_1.resetQuantityAdjustments)(target);
