...
-- File  : 102_patch_sp_update_stock_inventory_sku.sql
-- Target: sp_update_stock_inventory_sku
-- Executable: YES — uses T-SQL dynamic ALTER PROCEDURE
-- Prerequisites: 101_patch_sp_update_stock_inventory.sql must
--   have been executed first (creates sp_apply_utility_inventory_override).
-- Safety:
--   Idempotent. Skips if already patched or procedure not found.
--   No transaction history rows are modified.
--   No schema changes.
-- ============================================================

DECLARE @procName   NVARCHAR(128) = N'sp_update_stock_inventory_sku';
...
ELSE
BEGIN
  -- Normalize CREATE PROCEDURE / CREATE PROC to CREATE OR ALTER PROCEDURE
  ...
  -- Find last standalone \nEND using REVERSE trick.
  -- In the reversed string, \nEND becomes DNE\n.
  SET @revDef = REVERSE(@def);

  -- Try CHAR(10)+END (Unix newline)
  SET @revPos = PATINDEX(N'%' + N'DNE' + CHAR(10) + N'%', @revDef);

  IF @revPos > 0
  BEGIN
    -- insertPos = position of \n before END in original (1-based)
    SET @insertPos = LEN(@def) - @revPos - 2;
    SET @patched =
      LEFT(@def, @insertPos)
      + @patchBlock
      + SUBSTRING(@def, @insertPos + 1, LEN(@def));
  END
  ELSE
  BEGIN
    -- Fallback: try CHAR(13)+CHAR(10)+END (Windows newline)
    ...
    ELSE
    BEGIN
      -- No standalone END found; append at end.
