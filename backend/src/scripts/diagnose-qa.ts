...
interface LiveItemRow {
  ...
  itemname: string;
  end_qty: unknown;
  END_QTY_TEMP: unknown;
  ASSEMBLY_QTY: unknown;
}
...
function flag(value: unknown, label: string): string {
  if (value == null) return ` ← NULL treated as 0 by toNumber()  ⚠ (${label})`;
  if (Number(value) === 0) return ` ← zero  ⚠ (${label})`;
  return '';
}

async function diagnoseQa(qaNo: string): Promise<void> {
  ...
  const detailResult = await pool
    .request()
    .input('qaId', sql.BigInt, header.qa_id)
    .query<DetailRow>(`
      SELECT
        line_no,
        itemcode,
  ...
  const placeholders = itemcodes.map((code, i) => {
    req.input(`code${i}`, sql.NVarChar, code);
    return `@code${i}`;
  });

  const itemResult = await req.query<LiveItemRow>(`
    SELECT
      itemcode,
      itemname,
      end_qty,
  ...
  let zeroOldQtyLines: string[] = [];
  let nullEndQtyLines: string[] = [];
  let mathMismatchLines: string[] = [];
  ...
  for (const detail of details) {
    ...
    if (zeroOldFlag) zeroOldQtyLines.push(detail.itemcode);
    if (liveEndQty == null) nullEndQtyLines.push(detail.itemcode);
    if (!mathOk) mathMismatchLines.push(detail.itemcode);

    console.log();
    console.log(`  Line ${detail.line_no}: [${detail.itemcode}] ${detail.itemname}`);
    console.log(
      `    Stored old_qty:    ${fmt(detail.old_qty)}${flag(detail.old_qty, 'current qty at save/post time')}`
    );
    ...
    console.log(
      `    Requested qty:     ${requestedQty != null ? requestedQty.toFixed(2) : 'NULL (pre-migration record)'}`
    );
    ...
    if (!liveItem) {
      ...
      console.log(
        `    Live end_qty:      ${fmt(liveEndQty)}${liveEndQty == null ? '' : ''}`
      );
